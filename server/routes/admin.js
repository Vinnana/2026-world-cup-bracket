...
// ONE-TIME: flip home↔away goals for the 24 matches where our algorithmically-
// generated schedule had the wrong home team vs FIFA/ESPN official designation.
// Affected: MD2 pair2 (m4,m10,m16,m22,m28,m34,m40,m46,m52,m58,m64,m70)
//           MD3 pair1 (m5,m11,m17,m23,m29,m35,m41,m47,m53,m59,m65,m71)
// DELETE THIS ENDPOINT AFTER THE MIGRATION RUNS.
router.post('/migrate-homeaway', async (req, res) => {
  if (req.body.secret !== 'migrate-homeaway-2026') return res.status(403).json({ error: 'forbidden' })

  const FLIP = ['m4','m5','m10','m11','m16','m17','m22','m23',
                'm28','m29','m34','m35','m40','m41','m46','m47',
                'm52','m53','m58','m59','m64','m65','m70','m71']

  try {
    // ── 1. Flip picks in Postgres ─────────────────────────────────────────
    const pickRes = await db._pool.query(
      `UPDATE wc_score_picks
    ...
    // ── 2. Flip results in Postgres ────────────────────────────────────────
    // Also swap team names so the admin panel shows the correct sides.
    const scoreRes = await db._pool.query(
    ...
    // m4 edge case: admin entered home_team='Czechia' (already correct for new
    // schedule) so the swap above incorrectly swapped it to South Africa.
    // Re-fix: set Czechia=home, South Africa=away for m4.
    await db._pool.query(
      `UPDATE wc_match_scores
          SET home_team = 'Czechia',
              away_team = 'South Africa'
        WHERE match_id = 'm4'`
    )

    // ── 3. Update in-memory cache (avoid stale data until next restart) ────
    const flipSet = new Set(FLIP)

    db._data.score_picks = db._data.score_picks.map(p => {
      if (!flipSet.has(p.match_id)) return p
      return { ...p, home_goals: p.away_goals, away_goals: p.home_goals }
    })

    db._data.match_scores = db._data.match_scores.map(s => {
      if (!flipSet.has(s.match_id) || s.home_goals == null) return s
      // Swap goals
      const swapped = { ...s, home_goals: s.away_goals, away_goals: s.home_goals,
                               home_team: s.away_team,  away_team:  s.home_team }
      return swapped
    })
    // Apply m4 team-name correction to cache too
    db._data.match_scores = db._data.match_scores.map(s => {
      if (s.match_id !== 'm4') return s
      return { ...s, home_team: 'Czechia', away_team: 'South Africa' }
    })

    res.json({
      success: true,
      picks_rows_updated: pickRes.rowCount,
      result_rows_updated: scoreRes.rowCount,
      match_ids: FLIP,
    })
  } catch (err) {
    console.error('[admin] migrate-homeaway error:', err)
